"""Functions for common math operations."""


def add(num1, num2):
    """Return the sum of num1 and num2."""

    return  num1 + num2   # Returns num1 plus num2


def subtract(num1, num2):
    """Return the value of num1 minus num2."""
    return num1 - num2   # Returns num1 minus num2

def multiply(num1, num2):
    """Multiply the num1 by num2 and return the result."""
    return num1 * num2   # Returns num1 times num2

def divide(num1, num2):
    """Divide the num1 by num2, returning a floating point."""
    return num1 / num2   # Returns num1 divided by num2

def square(num1):
    """Return the square of num1."""
    return num1 ** 2   # Returns num1 squared

def cube(num1):
    """Return the cube of num1."""
    return num1 ** 3   # Returns num1 cubed

def power(num1, num2):
    """Raise num1 to the power of num2 and return the value."""
    return num1 ** num2    # returns num1 raised to num2

def mod(num1, num2):
    """Return the remainder of num1 / num2."""
    return num1 % num2   # Returns remainder of num1 / num2